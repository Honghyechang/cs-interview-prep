...
- **답변**: 하드웨어 인터럽트가 발생하면, cpu에게 인터럽트 서비스 요청 신호를를 보냅니다. 이때 cpu의 플래그 레지스터를 확인하여 인터럽트를 받을 수 있는 상태인지를 확인하고, 인터럽트 플래그가 활성화 되어있다면, 현재 진행중인 작업에 대한 데이터를 백업해놓습니다. 이후 인터럽트 요청을 보낸 대상으로부터 인터럽트 벡터를 받고, 이를 참조하여 인터럽트 서비스 루틴을 실행합니다. 인터럽트 서비스 루틴 실행이 끝나면 백업해둔 작업을 복구하여 실행합니다.
...
## 질문 39: 프로세스를 순차적으로(주소순대로) 실행될 수 있는 이유는 무엇인가요?
- **답변**: cpu안에 있는 pc 레지스터의 값이 1씩 증가하기 때문에 순차적으로 실행되는 것입니다.
## 질문 39-1: 그러면 PC의 값은 항상 증가하나요?
- **답변**: 아닙니다. 조건문이나, 반복문처럼 실행 흐름이 순차적이지 않을 때는 PC 값이 임의의 위치로 변경됩니다.

## 질문 40: 명령어를 효율적으로 처리하는 것과 하드웨어 인터럽트는 어떤 관계가 있나요?
- **답변**: 입출력 장치의 요청이 들어왔다고 해봅시다, 입출력 작업은 CPU 처리보다 현저히 느리기 때문에 인터럽트를 사용하지 않는다면 입출력 작업이 끝날때까지 CPU가 대기하거나 또는 주기적으로 끝났는지 확인해야하며 CPU 활용률이 떨어집니다. 반면 인터럽트를 사용하면 입출력 작업을 하는 동안에는 CPU가 다른 작업을 하고, 입출력 작업이 끝나면 인터럽트를 보내 CPU가 처리할 수 있어, CPU의 활용률이 높아집니다.


## 질문 41: 폴트와 트랩의 차이를 설명해주세요.
- **답변**: 폴트와 트랩 모두 동기 인터럽트에 속합니다. 다만 폴트는 폴트가 발생한 그 명령어부터 다시 실행해나가는 것이고, 트랩은 트랩이 발생한 다음 명령어부터 실행해나가는것입니다.(디버깅의 브레이크 포인트)
