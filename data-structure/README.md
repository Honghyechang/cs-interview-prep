...
- **답변**:equals()의 두 객체가 논리적으로 동등한지를 판단하고, hashCode()는 해시 기반 자료구조에서 객체를 저장할 버킷을 생성합니다. 만약 equals()는 재정의하여 논리적으로 동등한데, hashCode()를 재정의하지 않았다면 같은 키임에도 불구하고, hashcode의 결과가 달라서 다른 버킷에 저장되는 문제가 발생합니다.
...
## 질문 14:AVL 트리와 RB 트리에 대해서 설명해주세요.
- **답변**:AVL 트리와 RB트리 모두 편향된 이진 탐색 트리를 예방하기 위한 자가 균형 이진 탐색 트리입니다. AVL 트리는 모든 노드에서 왼쪽 서브트리와 오른쪽 서브트리의 높이의 차가 1이하를 유지하며, 균형 조건이 엄격합니다. 반면 RB트리는 Red와 Blue라는 색깔 속성을 이용하여 트리의 균형을 맞춥니다. AVL 트리에 비해 비교적 덜 엄격합니다.
...
## 질문 17: 우선순위큐는 어떤 자료구조로 구현되어있나요?
- **답변**:우선순위 큐는 힙(Heap) 자료구조로 구현되어 있습니다. 힙은 완전 이진 트리의 일종으로, 루트 노드는 가장 우선순위가 높은 값을 가지게 됩니다.
일반 큐(FIFO)와 달리, 우선순위 큐는 우선순위가 가장 높은 요소를 먼저 꺼낼 수 있으며, 삽입과 삭제 모두 **O(log n)**의 시간 복잡도를 가집니다.

## 질문 18:그래프의 종류를 설명해주세요(4가지)
- **답변**: 그래프는 크게 4가지로 분류됩니다. 간선의 방향이 있는지에 따라 방향그래프/무방향그래프, 모든 정점 쌍 사이에 경로가 존재하면 연결그래프, 존재하지 않으면 비연결그래프 , 간서에 가중치가 부여되는 가중치 그래프, 그래프의 정점과 간선 일부로 구성된 부분그래프가 존재합니다.

## 질문 19: 그래프를 구현하는 2가지 방법을 설명해주세요.
- **답변**: 그래프는 인접 행렬과 인접 리스트로 구현할 수 있습니다. 인접 행렬은 2차원 배열로 표현하며 간선 확인이 빠르나, 메모리 공간을 많이 차지합니다. 반면 인접 리스트는 연결 리스트로 표현하며 메모리 공간은 적으나, 간선 확인이 조금 더 오래걸립니다.

## 질문 20: 가중치 그래프는 어떻게 표현하나요?
- **답변**: 인접 행렬에서는 0,1 대신 가중치 값을 저장하고, 인접 리스트에서는 (정점,가중치)를 쌍으로 저장합니다.


## 질문 21: 인코딩된 값과 해시 값의 차이를 설명해보세요.
- **답변**: 인코딩은 데이터를 특정 형식으로 변환하는 양방향 과정으로 디코딩이 가능하지만, 해시는 데이터를 고정 길이 값으로 변환하는 단방향 과정으로 원본 복원이 불가능합니다.

## 질문 22: 깊이 우선 탐색과 너비 우선 탐색의 차이를 설명해주세요.
- **답변**: 깊이 우선 탐색(DFS)은 한 경로를 끝까지 깊게 탐색하고, 다시 돌아와서 다른 경로를 탐색하는 방식으로 스택이나 재귀함수로 구현합니다. 반면 너비 우선 탐색(BFS)는 시작 노드에서 가까운 노드부터 레벨 순서대로 탐색하는 방식으로 큐를 사용해 구현합니다.
